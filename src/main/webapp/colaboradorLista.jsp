...
<body>
    <div class="container mt-4">
        ...
        <div class="d-flex gap-3 mb-4">
            ...
            <div>
                <h2 class="fw-bold mb-0">Colaboradores</h2>
                <p class="text-muted mb-0">Gestiona la información de los colaboradores independientes.</p>
            </div>
        </div>

        <div class="d-flex gap-2 mb-4">
            <a href="dashboard" class="btn btn-outline-secondary"><i class="bi bi-arrow-left"></i> Volver al Dashboard</a>
            <a href="colaborador?accion=nuevo" class="btn btn-provital">
                <i class="bi bi-plus-lg"></i> Nuevo Colaborador
            </a>
        </div>

        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                <form action="colaborador" method="get" class="row g-2 align-items-center">
                    <div class="col">
        ...
        <div class="card border-0 shadow-sm">
            <table class="table table-hover mb-0" id="tablaColaboradores">
                <thead>
                    <tr>
                        <th></th>
                        <th>Nombre</th>
                ...
                <tbody>
                    <c:forEach var="col" items="${listaColaboradores}">
                        <tr data-categoria="${col.categoria}">
                            <td>
                                <div class="avatar-iniciales">${col.nombre.substring(0,1)}${col.apellido.substring(0,1)}</div>
                            </td>
                            <td>${col.nombre}</td>
                            <td>${col.apellido}</td>
                            <td>${col.ci}</td>
                            <td>${col.correo}</td>
                            <td>
                                <span class="badge rounded-pill ${col.categoria == 'manipulador' ? 'badge-manipulador' : 'badge-admisional'}">
                                    ${col.categoria == 'manipulador' ? 'Manipulador' : 'Admisional'}
                                </span>
                            </td>
                            <td>
                                <a href="persona?id=${col.idPersona}" class="btn btn-sm btn-outline-primary"><i class="bi bi-eye"></i> Ver Detalle</a>
                                <a href="colaborador?accion=editar&id=${col.idPersona}" class="btn btn-sm btn-outline-warning"><i class="bi bi-pencil"></i> Editar</a>
                                <a href="colaborador?accion=eliminar&id=${col.idPersona}"
                                   class="btn btn-sm btn-outline-danger"
                                   onclick="return confirm('¿Seguro que deseas eliminar este colaborador?');">
    ...
    <script>
        ...
        function renderizarTabla() {
            ...
            const todasLasFilas = Array.from(document.querySelectorAll('#tablaColaboradores tbody tr'));
            ...
            document.getElementById('infoPaginacion').textContent =
                ...
                Math.min(inicio + porPagina, filasCoincidentes.length) + ' de ' + filasCoincidentes.length + ' colaboradores';
        }
